...
public class AuthBO {
    ...
    public LoginResponse autenticar(LoginRequest req) {
        ...
        if (c != null) {
            return new LoginResponse("colaborador", c.getIdColaborador(), c.getNome(), c.getEmail(), c.getCargo());
        }

        Dentista d = buscarDentista(req.email(), req.senha());
        if (d != null) {
            return new LoginResponse("dentista", d.getIdDentista(), d.getNome(), d.getEmail(), null);
        }
